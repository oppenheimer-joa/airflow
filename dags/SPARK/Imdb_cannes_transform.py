...
default_args = {
    ...
    'start_date':datetime(1961, 7, 1, tzinfo=KST)}

dag = DAG('spark_Imdb_cannes_transform',
      default_args = default_args,
      max_active_runs= 1,
      tags=['스파크','IMDB transform','cannes'],
      schedule_interval= '0 16 1 7 *')
...
t1 = SSHOperator(
    task_id = 'sh_test',
    command = f"ssh sparkserver 'sh /home/spark/code/main/sh/imdb_transform_pyspark.sh {exe_date} busan' ",
    ssh_hook = sshHook2,
    do_xcom_push=False,
    dag=dag)
