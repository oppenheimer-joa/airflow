...
def get_api_data(date, **context):
    api_url = f"http://{SERVER_API}/tmdb/movie-details?date={date}"
...
def check_logic(category, date, **context):
    # XCom에서 값을 가져옵니다.
    ti = context['ti']
    xcom = ti.xcom_pull(task_ids='get_TMDB.Details_datas', key='db_counts')
    print(xcom)
...
start = EmptyOperator(task_id = 'start_TMDB.Details_task', dag = dag)
get_data = PythonOperator(task_id = "get_TMDB.Details_datas", python_callable=get_api_data, op_args=["{{execution_date.add(days=182, hours=9).strftime('%Y-%m-%d')}}"], dag = dag)
...
push_data = PythonOperator(task_id = "push_TMDB.Details_datas", python_callable=blob_data, op_args=["{{execution_date.add(days=182, hours=9).strftime('%Y-%m-%d')}}", f'http://{SERVER_API}/blob/tmdb'], dag = dag)
cleansing_data = PythonOperator(
    task_id = 'delete_TMDB.Details_datas',
    python_callable=erase_loaded_data,
    op_args=["{{execution_date.add(days=182, hours=9).strftime('%Y-%m-%d')}}"],
    dag = dag)

finish = EmptyOperator(task_id = 'finish_TMDB.Details_task', dag = dag)
