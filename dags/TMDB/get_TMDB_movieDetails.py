...
get_data = PythonOperator(task_id = "Get.TMDB_Details_Data", python_callable=get_api_data, op_args=["{{execution_date.add(days=182, hours=9).strftime('%Y-%m-%d')}}"], dag = dag)
...
push_data = PythonOperator(task_id = "Push.TMDB_Details_Data", python_callable=blob_data, op_args=["{{execution_date.add(days=182, hours=9).strftime('%Y-%m-%d')}}", f'http://{SERVER_API}/blob/tmdb'], dag = dag)
